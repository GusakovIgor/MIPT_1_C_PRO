...
#include "Headers/Parser.h"     // For SkipSpaces
...
bool SyntaxCheck   (String* buff, int type, ...)
{
    ...
    else if (type == WORD)
    {
        ...
        char* word = (char*) calloc (MAX_NAME_LEN, sizeof (char));

        int len = 0;
        sscanf (buff->pointer + buff->offset, "%[A-Za-z_]%n", word, &len);

        result = (strcmp (word, request) == 0) ? true : false;
...
void SyntaxCheckFailure (String* buff, int type, ...)
{
    ...
    else if (type == WORD)
    {
        ...
        sscanf (buff->pointer + buff->offset, "%[A-Za-z_]", word);
...
void PrintNodeLogs  (Node* node, FILE* GraphicLogs)
{
    if (node)
    {
        switch (node->type)
        {
            ...
            case (RET_TYPE):    fprintf (GraphicLogs, "\"%p\" [color = \"#000800\", fillcolor = green,    label = \"%s | %lg\"]\n",
                                                                                    node, "RET_TYPE", node->value.dbl_val);
                                break;
            ...
            case (COND_TYPE):   fprintf (GraphicLogs, "\"%p\" [color = \"#000800\", fillcolor = green,    label = \"%s | %s\"]\n",
                                                                                    node, "COND_TYPE", MathOutput (node->value.dbl_val));
                                break;
            ...
            case (VAR_TYPE):    fprintf (GraphicLogs, "\"%p\" [color = \"#000800\", fillcolor = blue,    label = \"%s | %s\"]\n",
                                                                                    node, "VAR_TYPE", node->value.str_val);
                                break;
