...
void Unit_Tests (int argc, const char* argv[])
{
    ...
    else if (strpbrk((*(argv + 1)), "12345") != 0)
    {
        for (int i = 1; i < argc; i++)
        {
            switch (atoi(*(argv + i)) )
            {
                ...
                default:    printf ("Wrong command: \"%s\", please enter this ones:\n", *(argv + i));
                            Help_Print ();
                            break;
            }
        }
    ...
    else
    {
        ...
        Help_Print();
        printf ("\nOr turn SUPERDEBUG mode off building stack with: \n");
        printf ("\"make Stack -f Makefile.make\"\n");
    }
}
...
void Test_StackTop ()
{
    ...
    for (int t = 0; t < num_tests; t++)
    {
        ...
        {
            printf ("#Test %d is Bad\n", t + 1);
            printf (" Expected top = %ld but recieved %f\n", num_test_el - 1, top);
        }
        else
        {
            printf ("#Test %d is Good\n", t + 1);
...
void Test_StackFree ()
{
    ...
    for (int t = 0; t < num_tests; t++)
    {
        ...
        {
          printf ("#Test %d is Bad\n", t + 1);
          printf (" Expected result capacity = %u  Recieved - %lu", result, test_stk->capacity);
        }
        else
        {
          printf ("#Test %d is Good\n", t + 1);
